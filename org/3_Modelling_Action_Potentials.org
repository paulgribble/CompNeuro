...
$m$ and $h$ representing the activation ($m$) and deactivation ($h$)
of the sodium channels. We have a differential equation for each,
...
$h$). Those equations involve parameters ($A$,$B$,$C$) that have
parameter values specific to $\alpha$ and $\beta$ (see Table 1 of
Ekeberg et al., 1991).
